...
#%%
imgs = data.sum(axis=(-1, -2))
vmin = np.min(imgs)
vmax = np.max(imgs)
ylim = np.array([34, 1])
fig, ax = plt.subplots(1, 5)
for i in range(5):
    ax[i].imshow(imgs[i], cmap='RdBu', vmin=vmin, vmax=vmax)
    ax[i].axis('off')
...
alphas[alphas > .38] = 1
alphas = 1 - alphas
for n, (img, alpha) in enumerate(zip(imgs, alphas)):
    # if n % 2 == 1:
    #     continue
    # n = n // 2
    # pcm = ax[n].imshow(img, cmap='RdBu', interpolation='bessel',vmin=vmin,vmax=vmax)
    pcm = ax[n].imshow(img, cmap='gray',vmin=0,vmax=1, alpha=alpha, aspect='auto')
    ax[n].set_xlabel(f'disp. (pm)\n\n{n - 2}V')
    ax[n].set_xticks([])
    ax[n].set_yticks([])
...
data_post_002_norm = fn_on_resized(data_post_002_norm, normalize_Data)
data_post_m002_norm = fn_on_resized(data_post_m002_norm, normalize_Data)
data_post_011_norm = fn_on_resized(data_post_011_norm, normalize_Data)
data_post_m011_norm = fn_on_resized(data_post_m011_norm, normalize_Data)
...
# vmin = np.min(output)
# output[lbl==0] = 0
sumI_002 = data_post_002_norm.sum(axis=(-1, -2))
sumI_002 = sumI_002 / sumI_002.max()
sumI_m002 = data_post_m002_norm.sum(axis=(-1, -2))
...
vmax = np.max(imgs)
vmin = np.min(imgs)
for n, img in enumerate(sumI_002):
    # if n % 2 == 1:
    #     continue
    # n = n // 2
    # pcm = ax[n].imshow(img, cmap='RdBu', interpolation='bessel',vmin=vmin,vmax=vmax)
...
# np.save('output/set1_Ru_002.npy', data_post_002_norm)
# np.save('output/set1_Ru_m002.npy', data_post_m002_norm)
