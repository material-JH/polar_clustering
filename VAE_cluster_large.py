...
from main import *
#%%
data_post_exp = np.load('output/set2_SRO_large.npy')
data_post_exp2 = np.load('output/set4_Ru_large.npy')
# data_post_exp = np.concatenate((data_post_exp, data_post_exp2), axis=0)

# %%
import atomai as aoi
imstack_train = data_post_exp[::2,::2,::2].reshape(-1, 50, 50)
imstack_train2 = data_post_exp2.reshape(-1, 50, 50)
#%%
# Intitialize rVAE model
input_dim = (50, 50)
rvae = aoi.models.rVAE(input_dim, latent_dim=2,
                        numlayers_encoder=3, numhidden_encoder=2 ** 8,
                        numlayers_decoder=3, numhidden_decoder=2 ** 8,)

if os.path.exists('output/rvae_large2.tar'):
    rvae.load_weights('output/rvae_large2.tar')
...
rvae.fit(
    ...
    batch_size=2 ** 8)
#%%
rvae.save_weights('output/rvae_large2')
...
for n in range(xyz):
    distances = np.linalg.norm(z33 - z13[n], axis=1) + abs(z31 - z11[n])
    if np.min(distances) > 0.04:
        continue
...
plt.show()
#%%
rvae.manifold2d(cmap='viridis', figsize=(8, 8), alpha=0.5, colorbar=True)
#%%
fig, axs = plt.subplots(nrows=1, ncols=5, figsize=(8, 4))
for n, img in enumerate(z23.reshape((*data_post_exp.shape[:3], 2))):
